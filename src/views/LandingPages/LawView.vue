...
// sections
import PresentationCounter from "@/views/LandingPages/Sections/PresentationCounter.vue";
import PresentationPages from "@/views/LandingPages/Sections/PresentationPages.vue";
import PresentationExample from "@/views/LandingPages/Sections/PresentationExample.vue";
import data from "@/views/LandingPages/Sections/Data/designBlocksData";
import BuiltByDevelopers from "@/views/LandingPages/components/BuiltByDevelopers.vue";
import PresentationTestimonials from "@/views/LandingPages/Sections/PresentationTestimonials.vue";
import PresentationInformation from "@/views/LandingPages/Sections/PresentationInformation.vue";

//images
import LawULogo from "@/assets/img/law-u-10-logo.png"
...
import lawData from "@/assets/json/law_data.json";
...
const getLawHeader = () => {
  ...
  return law ? law['header'] : '';
};

const getLawDetail = () => {
  const lawId = route.params.lawId;
  const law = lawData[lawId];
  return law ? law['detail'] : '';
};

const getClassDetail = (detail) => {
  const lawId = route.params.lawId;
  return detail == lawId ? 'lead text-white opacity-8' : 'text-white';
};
...
<template>
  ...
  <!-- Image carousel -->
  <Header>
    <div class="page-header min-vh-25" :style="`background-image: url(${vueMkHeader})`" loading="lazy">
      <div class="container">
        <div class="row">
          <div class="col-lg-7 text-center mx-auto position-relative">
            <h1 class="text-white pt-3 mt-n5 me-2" :style="{ display: 'inline-block ' }">
              <!-- LAW-U รออยู่ {{ $route.params.lawId }} -->
            </h1>
            <p class="lead text-white px-5 mt-3" :style="{ fontWeight: '500' }">
              <!-- Legal Chatbot for Sexual Violence -->
            </p>
          </div>
        </div>
      </div>
        ...
        <div class="container">
          <div class="row">
            <div class="col-lg-11 ms-lg-5">
              <h1 class="text-white">{{ getLawHeader() }}</h1>
              <br>
              <p class="lead text-white opacity-8" v-for="detail in getLawDetail()" :key="detail">
                &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;{{ detail }}
              </p>
              <br>
              <h6 class="text-white">
                อ้างอิงจาก
              </h6>
              <h6 class="text-white opacity-8" v-for="ref in getLawRef()" :key="ref">
                {{ ref }}
              </h6>
              <!-- <a
